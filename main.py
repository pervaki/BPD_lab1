...
class LoginApp:
    ...
    def login(self):
        ...
        if user["password_hash"] == "":
            self.master.withdraw()  # Скрываем основное окно
            self.set_initial_password(username)  # Установка нового пароля
            self.master.deiconify()  # Возвращаем основное окно
            return
...
class AdminApp:
    ...
    def lock_user(self):
        ...
        if confirm:
            ...
            messagebox.showinfo("Успіх",
                                f"Користувача '{user}' {'заблоковано' if self.users[user]['is_locked'] else 'розблоковано'}.")
    ...
    def toggle_password_restrictions(self):
        ...
        confirm = messagebox.askyesno("Підтвердження",
                                      f"Ви дійсно хочете {action.lower()} обмеження паролів для користувача '{user}'?")
        if confirm:
            self.users[user]["password_restrictions"] = not current
            self.save_func(self.users)
            self.refresh_user_list()
            messagebox.showinfo("Успіх",
                                f"Обмеження паролів для користувача '{user}' {'включено' if self.users[user]['password_restrictions'] else 'вимкнено'}.")
